...
<body>
    ...
    <div style="display:inline-flex; flex-direction:column; width:86%; height:auto; display:inline-flex; flex-direction:column; margin:0rem 7% 0rem; text-align:center;">
        <h5>학생들의 최근 체온 목록입니다.</h5>
        <form action="/m/record.php" method='post'>
                    ...
                    while($row1 = mysqli_fetch_assoc($result1)) {
                        ...
                        echo "
                            <tbody>
                                <tr>
                                    <td>".$cnt."</td>
                                    <td><button type='submit' name='student' style='border:0px solid rgba(0,0,0,0); background-color:rgba(0,0,0,0);' value='".$row1['mb_id']."'>".$row1['name']."(".$row1['code'].")</button></td>
                                    <td>".$row2['time']."</td>
                                    <td>".$row2['temp']."</td>
                                </tr>
                            </tbody>
        ...
        <h6>학생 이름을 터치하여 자세한 정보를 확인하세요.</h6>
        <form action="index.php" onsubmit="return logOut(this);" style="display:inline-flex; flex-direction:column; width:80%; height:auto; margin:0rem 10% 0rem;">
